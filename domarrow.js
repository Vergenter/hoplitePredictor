// from https://github.com/schaumb/domarrow.js
(()=>{
  ...
  function changedConnectionTag(changes) {
    changes.forEach(e => {
      var conn = e.target;
      if (conn.tagName.toLowerCase() !== 'connection-x' && e.attributeName === 'class')
        ;
      while (conn && conn.tagName.toLowerCase() !== 'connection-x')
  ...
  function bodyNewElement(changes) {
    changes.forEach(e => {
      e.removedNodes.forEach(n => {
        if (n.tagName && n.tagName.toLowerCase() === 'connection-x')
          removeconnection(n);
      });
